      SUBROUTINE XPRLS0(NOMA,FISS,NOESOM,ARMIN,CNSLN,ISOZRO,
     &                  NODTOR,ELETOR)
      IMPLICIT NONE
      CHARACTER*2    LEVSET
...
c    la partie suivante est suprime
...
C        CNSLT   :   CHAM_NO_S LEVEL SET NORMALE CALCULEE suprime
...
C      IF (NIV.GT.1)
      IF (NIV.GE.2) WRITE(IFM,*)'xprl0   CALCUL DES LEVEL SETS '
     &   //'A PROXIMITE DE L''ISOZERO DE '//LEVSET//'.'
...
c      IF (LEVSET.EQ.'LN') THEN
c         VNOULT = '&&XPRLS0.VNOULT'
c         CALL WKVECT(VNOULT,'V V R',NBNOCO,JNOULT)
c      ENDIF
...
C            IF (LEVSET.EQ.'LN') ZR(JNOULT-1+INO) = ZR(JLTNO-1+NUNO)
            GOTO 300
         ENDIF
               ...
               DO 340 INOA=1,NBNOMA
                  ...
                  IF (ABS(LSNA).LT.R8PREM()) THEN
                     ...
                     ELSEIF (NDIM.EQ.2) THEN 
                        Z(NPTINT) = 0.D0
                     ENDIF  
C                     IF (LEVSET.EQ.'LN') LST(NPTINT)=ZR(JLTNO-1+NUNOA)
...
C  ON VERIFIE LA VALIDITE DU POINT		     
 ...
 380                  CONTINUE
                     ...
                     IF (.NOT.DEJA) THEN
                       NPTINT = NPTINT+1
C                       IF (LEVSET.EQ.'LN') THEN
C                            LSTA = ZR(JLTNO-1+NUNOA)
C                            LSTB = ZR(JLTNO-1+NUNOB)
C                            LST(NPTINT) = LSTA + S*(LSTB-LSTA)
C                       ENDIF
                     ENDIF
                  ENDIF
...
C  ON STOCKE LA PREMIERE DISTANCE CALCULEE
                  IF (.NOT.DEJADI) THEN
                     BESTD = D
c                     IF (LEVSET.EQ.'LN')
c     &                  BESTLT = EPS(1)*LSTB + EPS(2)*LSTC + EPS(3)*LSTA
                     DEJADI=.TRUE.
                  ENDIF
...
C  ON STOCKE LA DISTANCE MINIMALE AVEC PROJECTION DANS LE TRIANGLE ABC
                  IF (IN) THEN
                     IF (.NOT.DEJAIN) THEN
                        BESTDI = D
c                        IF (LEVSET.EQ.'LN') BESTLI =
c     &                     EPS(1)*LSTB + EPS(2)*LSTC + EPS(3)*LSTA
                        DEJAIN=.TRUE.
                     ENDIF
                     IF (D.LT.BESTDI) THEN
                        BESTDI = D
...
c         IF (LEVSET.EQ.'LN') ZR(JNOULT-1+INO) = BESTLT
         IF (DEJAIN) THEN
            ZR(JNOULS-1+INO) = BESTDI*SIGN(1.D0,LSN)
c            IF (LEVSET.EQ.'LN') ZR(JNOULT-1+INO) = BESTLI
         ENDIF
...
C         IF (LEVSET.EQ.'LN') THEN
C            ZR(JLTNO-1+NUNO) = ZR(JNOULT-1+INO)
C         ENDIF
 
 400  CONTINUE

      IF (NIV.GT.1)
     & WRITE(IFM,*)'xprl0 NB DE'//
     &             ' NOEUD DE LEVEL SETS CALC :',NBNOCO+NBNOZO
...
C   DESTRUCTION DES OBJETS VOLATILES
      ...
      CALL JEDETR(NOMCOU)
      CALL JEDETR(VNOULS)
c      IF (LEVSET.EQ.'LN') CALL JEDETR(VNOULT)
