      SUBROUTINE XPRINI(MODEL,NOMA,CNXINV,FISPRE,FISS,CNSLN,
     &                  CNSGLS,NOESOM,NORESI,VCN,VCND,REFLOC)
      IMPLICIT NONE
      CHARACTER*8    MODEL,NOMA,FISPRE,FISS
      CHARACTER*19   CNSLN,CNSGLS,NOESOM,NORESI,VCN,VCND,
     &               CNXINV,REFLOC
...
C        CNSLT   : CHAM_NO_S DES VALEURS DE LEVEL SET TANGENTE supr
...
C-----------------------------------------------------------------------
      ...
      IF (NIV.GE.2) WRITE(IFM,*)'xprini debug   XPRINI debut '
...
c      CALL JEVEUO(CNSLT//'.CNSV','E',JLTNO)
      ...
      IF (NIV.GE.2) WRITE(IFM,*)'xprini debut XPRINI debut de simplexe'
C-----------------------------------------------------------------------
...
C------------------------------------------------------------------
      ...
      DO 100 INO=1,NBNO
         ...
         IF (NOMNO(1:2).EQ.'NS') THEN
	      WRITE(*,*)'on a un noeud sommet INO',INO
	      ZL(JNOSOM-1+INO) = .FALSE.
         ENDIF
...
C  SI LA MAILLE EST COUPEE PAR LES ISOZEROS DE LS 
         IF (COUPLN) THEN
            NBMAFF = NBMAFF + 1
...
C  LE NOEUD EST PROCHE D'UNE MAILLE DU FOND DE FISSURE
 ...
 250  CONTINUE
      ...
      IF (NIV.GE.2) THEN
         WRITE(IFM,*)'xprini NOMBRE DE NOEUDS POUR L'''
