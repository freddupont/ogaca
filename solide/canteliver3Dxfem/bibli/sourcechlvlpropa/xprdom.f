      SUBROUTINE XPRDOM(DNOMO,DNOMA,DCNXIN,NOMO,NOMA,CNXINV,FISS,
...
C    suppr DISFR  = NOM DU VECTEUR JEVEUX OU LES DISTANCE^2 ENTRE LES NOEUDS
C              DU MAILLAGE DNOMA ET LE FOND DU FISSURE SONT STOCKEES.
C              CE VECTEUR EST DONNE PAR XPRVIT.F.

C     Il faut faire en sorte que la selection soit plustot sur la valeur de la level set.
C     pas encore au point la fonction est desactive.
C    * MODELE PHYSIQUE
C      ---------------
...
c      CALL JEVEUO(FISS//'.FONDFISS','L',JFONF)
c      CALL JELIRA(FISS//'.FONDFISS','LONMAX',LONG,K1BID)
c      NBPTFF=LONG/4

C     RETRIEVE THE DIFFERENT PIECES OF THE CRACK FRONT
c      CALL JEVEUO(FISS//'.FONDMULT','L',JFMULT)
c      CALL JELIRA(FISS//'.FONDMULT','LONMAX',NUMFON,K1BID)
c      NUMFON = NUMFON/2
