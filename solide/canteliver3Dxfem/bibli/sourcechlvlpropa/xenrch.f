...
C-------------------------------------------------------------------
      ...
      IF (NIV.GE.3) 
     &  WRITE(IFM,*)'xenrch 1) RESTRICTION DE LA ZONE D ENRI'
      MAFIS='&&XENRCH.MAFIS'
...
C--------------------------------------------------------------------
      ...
      IF (NIV.GE.3) 
     &   WRITE(IFM,*)'xenrch2) ATTRI DU STAT DES Nd DE GROUPENRI'
...
C     CALCUL DU STATUT DES NOEUDS
     ...
     &                                                          STANO)
C      CALL XSTANO(NOMA,LISNOE,NMAFIS,JMAFIS,CNSLN,CNSLN,RAYON,CNXINV,
C     &                                                          STANO)
...
C     POUR UNE INTERFACE, ON PASSE DIRECTEMENT A LA CREATION DE LA SD 
      IF (TYPDIS.EQ.'INTERFACE') THEN
        IF (NIV.GE.3) 
     &        WRITE(IFM,*)'xenrch  debug c une interface'

C        CALL ASSERT(NMAEN2+NMAEN3.EQ.0)
...
C--------------------------------------------------------------------
      ...
      IF (NIV.GE.3) WRITE(IFM,*)'xenrch 6) CALCUL DES FONDS MULT'
...
C--------------------------------------------------------------------
      ...
      IF (NDIM.EQ.3) THEN
        ...
        IF (NIV.GE.3) WRITE(IFM,*)'xenrch 6) CALCUL DES ABSCISSES CURV'
...
C
      ...
      IF (NIV.GE.3) WRITE(IFM,*)'xenrch 7) FIN DE XENRCH'
